...
test('sessionStart injects the default-level ruleset as additional_context and keeps state under ~/.cursor', () => {
  ...
  assert.match(output.additional_context, /Challenge existing representations/, 'ultra row must survive the level filter');
  assert.doesNotMatch(output.additional_context, /Apply these criteria/, 'lite row must be filtered out');
...
test('beforeSubmitPrompt tracks /ponytail commands and delivers the new level ruleset', () => {
  ...
  assert.match(sw.additional_context, /Apply these criteria/, 'Cursor has no /ponytail command, so the level ruleset rides along');
  assert.doesNotMatch(sw.additional_context, /Challenge existing representations/);
